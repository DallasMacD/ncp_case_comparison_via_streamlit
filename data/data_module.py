...
def extract_financials(directory: str):
	...
	if os.path.isfile(file_path):
		temp_df = pd.read_csv(file_path)
		revenue = round(abs(temp_df.loc[temp_df['Values of the Objective Function Terms'].str.contains('Elastic Demand|Revenue'), '(k$)'].sum()), 2)
		costs = -round(abs(temp_df.loc[temp_df['Values of the Objective Function Terms'].str.contains('Cost'), '(k$)'].sum()), 2)
		violations = round(abs(temp_df.loc[temp_df['Values of the Objective Function Terms'].str.contains('Violation'), '(k$)'].sum()), 2)
