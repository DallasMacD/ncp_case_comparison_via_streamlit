...
#--------------------------------------------------------------------------------------------------#
	...
	def create_single_case_line_chart(primary_axis_df: pd.DataFrame, secondary_axis_df: pd.DataFrame):
		...
		for case_name in case_list:
			master_fig = make_subplots(subplot_titles=[f'<b>{case_name}</b>'], specs=[[{'secondary_y': True}]])
			...
			if primary_axis_empty and not secondary_axis_empty:
				combined_case_chart_df = secondary_axis_case_df.copy()
			if not primary_axis_empty and secondary_axis_empty:
				combined_case_chart_df = primary_axis_case_df.copy()
			if not primary_axis_empty and not secondary_axis_empty:
				combined_case_chart_df = primary_axis_case_df.merge(right=secondary_axis_case_df, on=['Hour', 'Case Name'], how='outer')
			...
			with axis_expander_column:
				with st.expander('Update Axis Bounds'):
					...
					with axis_label_column:
						st.text_input('Primary Y-Axis Label', value='Primary Y-Axis', key=f'y1_label_text_input_{case_name}', disabled=True, label_visibility='collapsed')
						st.text_input('Secondary Y-Axis Label', value='Secondary Y-Axis', key=f'y2_label_text_input_{case_name}', disabled=True, label_visibility='collapsed')
						st.text_input('X-Axis Label', value='X-Axis', key=f'x1_label_text_input_{case_name}', disabled=True, label_visibility='collapsed')
					with axis_lower_bounds_column:
						y1_lower_bound_input = st.number_input(label='Lower Bound', disabled=primary_axis_empty, value=y1_lower_bound, key=f'y1_lower_bound_number_input_{case_name}', label_visibility='collapsed')
						y2_lower_bound_input = st.number_input(label='Lower Bound', disabled=secondary_axis_empty, value=y2_lower_bound, key=f'y2_lower_bound_number_input_{case_name}', label_visibility='collapsed')
						x1_lower_bound_input = st.number_input(label='Lower Bound', value=x1_lower_bound, key=f'x1_lower_bound_number_input_{case_name}', label_visibility='collapsed')
					with axis_upper_bounds_column:
						y1_upper_bound_input = st.number_input(label='Upper Bound', disabled=primary_axis_empty, value=y1_upper_bound, key=f'y1_upper_bound_number_input_{case_name}', label_visibility='collapsed')
						y2_upper_bound_input = st.number_input(label='Upper Bound', disabled=secondary_axis_empty, value=y2_upper_bound, key=f'y2_upper_bound_number_input_{case_name}', label_visibility='collapsed')
						x1_upper_bound_input = st.number_input(label='Upper Bound', value=x1_upper_bound, key=f'x1_upper_bound_number_input_{case_name}', label_visibility='collapsed')
...
#--------------------------------------------------------------------------------------------------#
	...
	# Initialize Session State variables to store variables across mulitple pages and app reruns
	if 'line_chart_case_selection' not in st.session_state:
		st.session_state.line_chart_case_selection = None
		st.session_state.line_chart_primary_metric_selection = None
		st.session_state.line_chart_primary_agent_options = []
		st.session_state.line_chart_primary_agent_selection = None
		st.session_state.line_chart_primary_filtered_metrics_df = pd.DataFrame()
		st.session_state.line_chart_secondary_metric_selection = None
		st.session_state.line_chart_secondary_agent_options = []
		st.session_state.line_chart_secondary_agent_selection = None
		st.session_state.line_chart_secondary_filtered_metrics_df = pd.DataFrame()
	...
	with st.container():
		...
		if st.session_state.data_progress == 100:
			...
			selected_case = st.multiselect('Case Select', 
										...
										default=st.session_state.line_chart_case_selection,
										placeholder='Select a Case',
										key='line_chart_case_multiselect')

			# Metric Select - Create 2 Multiselect widgets to select metrics to plot on either the primary or secondary axes
			primary_metric_column, secondary_metric_column = st.columns(2)
			with primary_metric_column:
				selected_primary_metric = st.multiselect('Metric Select', 
											   			options=st.session_state.metric_list, 
														default=st.session_state.line_chart_primary_metric_selection,
														placeholder='Select a Primary Axis Metric',
														key='line_chart_primary_metric_multiselect', 
														on_change=update_agent_multiselect, 
														kwargs=dict(self_multiselect_key='line_chart_primary_metric_multiselect',
																	agent_multiselect_key='line_chart_primary_agent_multiselect',
																	agent_session_state_key='line_chart_primary_agent_selection',
																	agent_options_session_state_key='line_chart_primary_agent_options'))
			with secondary_metric_column:
				selected_secondary_metric = st.multiselect('Metric Select', 
											   			options=st.session_state.metric_list,
														default=st.session_state.line_chart_secondary_metric_selection,
														placeholder='Select a Secondary Axis Metric',
														key='line_chart_secondary_metric_multiselect',
														label_visibility='hidden',
														on_change=update_agent_multiselect, 
														kwargs=dict(self_multiselect_key='line_chart_secondary_metric_multiselect',
																	agent_multiselect_key='line_chart_secondary_agent_multiselect',
																	agent_session_state_key='line_chart_secondary_agent_selection',
																	agent_options_session_state_key='line_chart_secondary_agent_options'))

			# Agent Select - Create 2 Multiselect widgets to select agents to plot on either the primary or secondary axes
			primary_agent_column, secondary_agent_column = st.columns(2)
			with primary_agent_column:
				selected_primary_agent = st.multiselect('Agent Select', 
											  			options=st.session_state.line_chart_primary_agent_options, 
														default=st.session_state.line_chart_primary_agent_selection,
														placeholder='Select a Primary Axis Agent',
														key='line_chart_primary_agent_multiselect')
			with secondary_agent_column:
				selected_secondary_agent = st.multiselect('Agent Select', 
														options=st.session_state.line_chart_secondary_agent_options, 
														default=st.session_state.line_chart_secondary_agent_selection,
														placeholder='Select a Secondary Axis Agent',
														key='line_chart_secondary_agent_multiselect',
														label_visibility='hidden')

			# Apply Filters - Create a button to apply the above filters
			apply_filters_button = st.button('Apply Filters', 
											 use_container_width=True,
											 key='line_chart_apply_filters',
											 on_click=filter_metrics_df,
											 kwargs=dict(case_multiselect_key='line_chart_case_multiselect',
														case_session_state_key='line_chart_case_selection',
														primary_metric_multiselect_key='line_chart_primary_metric_multiselect',
														primary_metric_session_state_key='line_chart_primary_metric_selection',
														primary_agent_multiselect_key='line_chart_primary_agent_multiselect',
														primary_agent_session_state_key='line_chart_primary_agent_selection',
														secondary_metric_multiselect_key='line_chart_secondary_metric_multiselect',
														secondary_metric_session_state_key='line_chart_secondary_metric_selection',
														secondary_agent_multiselect_key='line_chart_secondary_agent_multiselect',
														secondary_agent_session_state_key='line_chart_secondary_agent_selection',
														primary_filtered_metrics_df_session_state_key='line_chart_primary_filtered_metrics_df',
														secondary_filtered_metrics_df_session_state_key='line_chart_secondary_filtered_metrics_df'))
			...
			if (not st.session_state.line_chart_primary_filtered_metrics_df.empty or not st.session_state.line_chart_secondary_filtered_metrics_df.empty):
				create_single_case_line_chart(primary_axis_df=st.session_state.line_chart_primary_filtered_metrics_df,
											secondary_axis_df=st.session_state.line_chart_secondary_filtered_metrics_df)
